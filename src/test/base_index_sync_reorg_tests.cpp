...
public:
		TestBlockFilterIndex(std::unique_ptr<interfaces::Chain> chain,
    ...
    if (!m_synced) {
        ...
        while (true) {
            if (pindex && pindex->nHeight == height) {
                ...
                sync_cv.wait(lock, [this, &reorg_done] { return reorg_done.load(); });
            }
...
BOOST_FIXTURE_TEST_CASE(blockfilter_index_sync_reorg, BuildChainTestingSetup)
{
    ...
    int pause_height = 97;
    vector<BlockIndex*> un_indexed;
    auto& chain = m_node.chainman->ActiveChain();
    for (int i = pause_height + 1; i < chain.size(); i++) {
