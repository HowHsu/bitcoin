    ...
    std::optional<R> LoopAtomic(bool fMaster, int id) EXCLUSIVE_LOCKS_REQUIRED(!m_mutex)
    {
        ...
        while (true) {
            if (!fMaster) {
                ...
                if (!fComplete || m_done[id]) {
                    nIdle++;
                    if (nIdle == (int)m_worker_threads.size())
                        m_master_cv.notify_one();
            ...
            if (fMaster) {
                WAIT_LOCK(m_mutex, lock);
                cond.wait(lock, [&] { return nIdle == (int)m_worker_threads.size(); });
...
public:
        ...
        : nBatchSize(worker_threads_num <= MAX_SCRIPTCHECK_MUTEX_MODE_THRESHOLD ? batch_size : 5)
    {
        ...
        if (worker_threads_num > MAX_SCRIPTCHECK_MUTEX_MODE_THRESHOLD) {
            fAtomic = true;
            m_done.resize(worker_threads_num, false);
        }
    ...
    std::optional<R> Complete() EXCLUSIVE_LOCKS_REQUIRED(!m_mutex)
    {
        ...
        WITH_LOCK(m_mutex, fComplete = true);
        if (queue.size() > 1)
            m_worker_cv.notify_all();
        auto ret = LoopAtomic(true /* master thread */, 0);
        nNext = 0;
        queue.clear();
        {
            LOCK(m_mutex);
