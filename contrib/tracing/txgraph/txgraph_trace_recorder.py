...
Requires bitcoind built with -DBUILD_TXGRAPH_REPLAY=ON so that bitcoind
waits for the tracer to attach before mempool initialization, ensuring
a complete trace starting from the INIT event.
...
def generate_bpf_program():
    ...
    for name, opcode, nargs, varlen in PROBES:
        ...
        lines.append(f"    struct txgraph_event e = {{}};")
        lines.append(f"    e.opcode = {opcode:#04x};")
        for i in range(nargs):
            lines.append(f"    bpf_usdt_readarg({i+1}, ctx, &e.{arg_fields[i]});")
        if varlen:
            lines.append(f"    u64 ptr = 0;")
            lines.append(f"    bpf_usdt_readarg(3, ctx, &ptr);")
            lines.append(f"    if (ptr != 0) {{")
            lines.append(f"        bpf_probe_read_user(e.elements,")
            lines.append(f"                            sizeof(e.elements),")
            lines.append(f"                            (void*)ptr);")
            lines.append(f"    }}")
        lines.append(f"    events.perf_submit(ctx, &e, sizeof(e));")
