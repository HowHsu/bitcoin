...
# Variable-length opcodes: [u32 count][u32 * count][u8 level]
VAR_LENGTH_OPS = {GET_ANCESTORS_UNION, GET_DESCENDANTS_UNION, COUNT_DISTINCT}


def read_var_payload(f):
    """Read variable-length payload: u32 count, count*u32 indices, u8 level."""
    raw = f.read(4)
    if len(raw) < 4:
        return False
...
def main():
    ...
    with open(sys.argv[1], 'rb') as f:
        ...
        print_report(graph, f"Final state", max_cluster_count)
